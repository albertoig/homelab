...
- [ ] Authenticate OpenBao metrics scraping — `openbao.yaml.gotmpl` sets `unauthenticated_metrics_access = true` so the prometheus serviceMonitor can scrape `/v1/sys/metrics` without a token; review switching to a scoped metrics token (or Kubernetes auth) so the endpoint is no longer exposed unauthenticated
